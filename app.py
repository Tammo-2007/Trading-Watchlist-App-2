...
if st.sidebar.button("Hinzufügen"):
    if ticker_input and st.session_state.active_mandant:
        ...
        st.warning("Bitte Mandanten auswählen und Ticker eingeben.")
...
with tab1:
    ...
    if portfolio.empty:
        ...
        def compute_values(row):
            ...
            gewinn = positionswert - (row["Kaufpreis"]*row["Stückzahl"])
...
with tab2:
    ...
    if portfolio.empty:
        st.info("Bitte zuerst Aktien hinzufügen.")
    else:
        ...
        if selected_ticker:
            ...
            if not data_hist.empty:
                ...
                **Legende:**  
                🔵 Blau = Schlusskurs  
                🟠 Orange = SMA20 (20-Tage gleitender Durchschnitt)  
                🟢 Grün = SMA50 (50-Tage gleitender Durchschnitt)
                """)
            else:
