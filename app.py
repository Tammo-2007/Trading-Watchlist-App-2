...
cols = st.columns([1,2])  # linke Spalte schmaler
with cols[0]:
    st.subheader("📋 Portfolio")
    for _, row in portfolio.iterrows():
        gain_color = "#008000"  # dunkelgrün
        loss_color = "#cc0000"  # dunkelrot
        ticker_color = "#003366"  # dunkelblau
        value_color = gain_color if row["Gewinn/Verlust"] >= 0 else loss_color
        st.markdown(f"""
        <div style="border:1px solid #999; padding:10px; border-radius:10px; margin-bottom:10px; background-color:#f0f0f0;">
